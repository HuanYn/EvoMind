...
已在单张 RTX 3090 上通过真实图文两次优化器更新预检，microbatch 4、累积 1、BF16；可训练参数为 15,931,776，探针峰值 allocated 2,013.97 MiB、reserved 2,248 MiB。两次探针 loss 为 3.1918、3.1145，只用于验证加载、前向、反向和显存，不作为质量或稳定吞吐结果。

首次预检在 CUDA 上下文尚未初始化时重置显存统计而退出，没有更新权重；修正初始化顺序与 Torch/NVML 的 UUID 前缀兼容后，保留失败现场并使用新运行目录。30 项启动器单元测试及一项原生 CPU 训练/恢复集成测试通过。

正式进程已重新加载未受探针更新影响的 CISPO 权重，完整读取 2,544,979 条训练记录，并实际完成首个优化器更新（训练 CE 3.2035）。预算为每轮 636,245 次更新、共两轮；首步包含冷启动开销，不据此给出训练 ETA。

训练入口、参数解释、曲线/快照与恢复约束见 [evomind-v 原生单图基线](https://github.com/HuanYn/evomind/tree/evomind-v#原生单图基线)。此处验收的是正式训练成功启动，不是训练完成或视觉效果达标。
