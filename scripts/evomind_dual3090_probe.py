...
RUN = ROOT / "artifacts/runs/dual3090_cispo_preflight_gpu20_20260909"
GPUS = {2: "GPU-3312e52e-57b1-ea7e-e2cf-fb4d86da3392", 0: "GPU-00f6cba0-232f-a1b3-a09d-1b4edc7861df"}
...
def prepare():
    ...
    atomic_json(RUN / "manifest.json", {"created_at": now(), "probe_only": True,
        ...
        "reason": "Check dual GPU full configured G/length memory and update integration, not final selected-base training",
        "adaptation": "Reward CPU FP32 -> GPU0 FP32; policy/reference GPU2 BF16; no data/G/length reduction; GPU3 newly occupied by another user"})
    print(RUN / "manifest.json", flush=True)


def run():
    if os.environ.get("CUDA_VISIBLE_DEVICES") != ",".join(GPUS.values()):
        raise RuntimeError("Must bind exactly authorized free physical GPUs 2 and 0 by UUID")
    ...
    with exclusive_run_lock(RUN):
        ...
        branch["options"].update(reward_model_path=str(ROOT / "models/internlm2-1_8b-reward"),
            reward_device="cuda:1", reward_dtype="float32", device="cuda:0")
